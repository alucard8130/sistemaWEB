...
class LocalComercialForm(forms.ModelForm):
    class Meta:
        ...
        fields = [
            "numero", "propietario", "cliente", "empresa", "tipo_propiedad",
            "superficie_m2", "cuota", "giro", "ubicacion", "status",
            "observaciones", "es_cuota_anual",
        ]
        widgets = {
            "numero": forms.TextInput(attrs={"class": "form-control", "placeholder": "Número, Codigo o Id."}),
            "propietario": forms.TextInput(attrs={"class": "form-control", "placeholder": "Propietario"}),
            "cliente": forms.Select(attrs={"class": "form-control"}),
            "empresa": forms.Select(attrs={"class": "form-control"}),
            "tipo_propiedad": forms.Select(attrs={"class": "form-control"}),
            "superficie_m2": forms.TextInput(attrs={"class": "form-control", "placeholder": "Superficie_m2"}),
            "cuota": forms.TextInput(attrs={"class": "form-control", "placeholder": "Importe Cuota Mensual"}),
            "es_cuota_anual": forms.CheckboxInput(attrs={"class": "form-check-input", "style": "margin-top: 0.3rem;"}),
            "giro": forms.TextInput(attrs={"class": "form-control", "placeholder": "Giro"}),
            "ubicacion": forms.Textarea(attrs={"rows": 2, "class": "form-control", "placeholder": "Ubicación"}),
    ...
    def __init__(self, *args, **kwargs):
        ...
        if user and not user.is_superuser:
            ...
            self.fields["cliente"].queryset = Cliente.objects.filter(empresa=empresa_usuario)
        else:
        ...
        if empresa_usuario and empresa_usuario.segmento == 'habitacional':
            ...
            self.fields['numero'].widget.attrs['placeholder'] = "Número de casa / departamento"
            ...
            self.fields['tipo_propiedad'].choices = [
                c for c in LocalComercial.TIPO_CHOICES if c[0] in ('casa', 'departamento')
            ]
        else:
            # Segmento comercial (o superusuario sin empresa fija todavía):
            # solo mostrar tipos de propiedad comerciales
            self.fields['tipo_propiedad'].choices = [
                c for c in LocalComercial.TIPO_CHOICES if c[0] in ('local', 'oficina', 'bodega', 'terreno')
            ]
