    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      var calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        eventClick: function(info) {
          ...
          // Busca el evento en la lista y resáltalo
          var li = document.querySelector('.list-group-item[data-id="' + info.event.id + '"]');
          if (li) {
            li.classList.add('selected');
            li.scrollIntoView({behavior: "smooth", block: "center"});
          } else {
            // Si el evento no está en la página actual, podrías mostrar un mensaje o paginar automáticamente
            alert('El evento seleccionado no está visible en el periodo actual.');
          }
        },
        eventDidMount: function(info) {
      // Muestra el título completo como tooltip nativo
